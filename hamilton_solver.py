...
class HamiltonSolver:
    ...
    def longest_path_to_tail(self,start_node):
        ...
        for next_node in lis:
            if next_node[-1]==self.apple_pos:
                next_node.insert(0,current[0])
            if len(self.shortest_path(next_node,next_node[0]))>1:
                path.append(next_node)
                break
        return path

    def hamilton(self,start_node,goal):
        path=self.shortest_path(start_node,goal)

        if len(path)>1 and len(start_node)<=(setting.GRID_WIDTH*setting.GRID_HEIGHT)//2:
    ...
    def update(self):
        if not self.game_over:
            path = self.hamilton(self.snake_pos, self.apple_pos)
            if path==None:
                return
            if len(path) > 1:
                next_pos = path[1][-1]
